...
parser.add_argument("proj", type=str, help="Dimension to project along: 'x', 'y', or 'z'")
...
while True:
    ...
    if i == np.prod(xyz_size):
        ...
        if j >= START_FRAME:
            ...
            for x in range(xyz_size[dimX]):
                for y in range(xyz_size[dimY]):
                    if S[x, y] > 0.05:
                        ...
                        y0 = qY[x, y] - C * dY[x, y]
                        y1 = qY[x, y] + C * dY[x, y]
                        ax.plot([x0, x1], [y0, y1], color='black', lw=MYLW, zorder=2)
            ...
            ax.axis('off')
            plt.savefig(f"{data_path}/frame{j:04d}.png", format='png', bbox_inches='tight', transparent=makeTransparent)
            if savePDF:
                plt.savefig(f"{data_path}/frame{j:04d}.pdf", format='pdf', bbox_inches='tight', transparent=makeTransparent)
            plt.close()
