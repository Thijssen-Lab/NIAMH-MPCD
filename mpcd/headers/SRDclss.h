...
typedef struct specSwimmer {
	...
	double sig;                 ///< LJ sigma --- diameter approx sigma - json `'sigSwim'`
    double isig;			    ///< Inverse LJ sigma
	double eps;					///< LJ interaction energy - json `'epsSwim'`
	double runTime;             ///< Average run time in units of MPC time steps (dt) - json `'runTSwim'`
    double tumbleTime;	        ///< Average tumble time in units of MPC time steps (dt) - json `'tumTSwim'`
...
typedef struct swimmer {
	...
	double ro;                  ///< FENE seperation - Each swimmer needs it's own since it changes for run/tumbler
    double iro;				    ///< Inverse FENE separation
	double sig;                 ///< LJ sigma - Diameter approx sigma - Each swimmer needs it's own since it changes for run/tumbler
    double isig;			    ///< Inverse LJ sigma
